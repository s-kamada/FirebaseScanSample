...
object CameraPermission {
    ...
    class RequestContract : ActivityResultContract<Unit, Boolean>() {
        ...
        override fun createIntent(context: Context, input: Unit?): Intent = Intent(ACTION).putExtra(
            EXTRA_REQUEST, arrayOf(PERMISSION))

        override fun parseResult(resultCode: Int, intent: Intent?): Boolean {

            if (resultCode != AppCompatActivity.RESULT_OK) return false

            return intent?.getIntArrayExtra(EXTRA_RESULT)?.getOrNull(0) == PackageManager.PERMISSION_GRANTED
        }
